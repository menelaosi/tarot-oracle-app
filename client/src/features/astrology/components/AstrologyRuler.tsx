import { CUSPS_STROKE, getPointPosition } from '../lib/horoscope';
...
function getRulerPositions(
  ...
  const halfRayRadius =
    startRadius <= endRadius ? rayRadius - radiusRatio : rayRadius + radiusRatio;
  const resultArray = [];
  for (let i = 0, start = 0, step = 5; i < 72; i++) {
    const angle = start + startAngle;
    const startingPoint = getPointPosition(point, startRadius, angle);

    const endPositionStartRadius = i % 2 === 0 ? rayRadius : halfRayRadius;

    const endingPoint = getPointPosition(point, endPositionStartRadius, angle);
